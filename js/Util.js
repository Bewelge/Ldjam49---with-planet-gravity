...
export function getBase64Buffer(str) {
	let base64 = str.split(",")[1]
	return Base64Binary.decodeArrayBuffer(base64)
}

function strokeLine(c, vec1, vec2, lineWidth, color) {
	c.lineWidth = lineWidth
	c.strokeStyle = color
	c.beginPath()
	c.moveTo(vec1.x, vec1.y)
	c.lineTo(vec2.x, vec2.y)
	c.stroke()
	c.closePath()
}
function fillArc(c, x, y, rad) {
	c.beginPath()
	c.moveTo(x, y)
...
export function star(ct, x, y, rad) {
...
export const appendBody = el => {
	document.querySelector("#mainCont").appendChild(el)
}
export const removeBody = el => {
	document.querySelector("#mainCont").removeChild(el)
}
export const rndInt = (min, max) => Math.floor(rndFloat(min, max))
export const rndFloat = (min, max) => min + (max - min) * Math.random()
export {
	formatTime,
	arrayContains,
	drawRoundRect,
